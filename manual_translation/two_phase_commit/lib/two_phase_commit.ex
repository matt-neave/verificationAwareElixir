...
defmodule Client do
  ...
  @ltl "<>(commits == n_ps || aborts == n_ps)"
  def await_transaction_result(commits, aborts, n_ps) do
    if commits + aborts >= n_ps do
      IO.puts("All participants have responded")
    else
      receive do
        {:transaction_commit} ->
          await_transaction_result(commits + 1, aborts, n_ps)
        {:transaction_abort} ->
          await_transaction_result(commits, aborts + 1, n_ps)
      end
    end
  end
end
