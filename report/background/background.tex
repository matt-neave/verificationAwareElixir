...
\par
We can now introduce the process algebra for concurrency, using the parallel composition operator (`||'). To help reason about concurrent processes, we also introduce $\mu X \bullet F(X)$ to define anonymous recursive processes (this is named the fixed-point operator and it will become clear how this simplifies notation through examples). This now lets us denote a process that behaves like a system of composed processes, where both processes have the same algebra $\alpha S$.
\[
(T \,||\, L)    
\]
...
\subsection{Linear Time Temporal Logic TODO}
