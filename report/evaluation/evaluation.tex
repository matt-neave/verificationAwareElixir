...
\begin{itemize}
    ...
    \item A philosopher will attempt to pick up the to their left.
...
\end{itemize}
In particular, the process algebra for a philosopher, $P$ is as follows:
\[
\begin{aligned}
    & \text{P} = ( \text{sit} \rightarrow \text{pickupLeft} \rightarrow \text{pickupRight} \rightarrow \text{eat} \rightarrow \text{putdownLeft} \rightarrow \text{putdownRight} \rightarrow \text{leave} \rightarrow \text{SKIP}) \\
\end{aligned}
\]
...
\par
To summarise the trail produced by Verlixir, we will demonstrate a process composition for two philosophers, $Phil_0$ and $Phil_1$ that results in a deadlock. We denote the fork on the philosopher's left as as $Fork_{i}$ and on the right as $Fork_{(i+1)\%n}$ for philosopher $Phil_i$, where $n$ is the number of philosophers (similarly for a fork, the philosophers are located at $i$ and $(i-1)\%n$) We will use $L$ and $R$ interchangeably to express left and right. 
\[
\begin{aligned}
...
\subsection{Raft Consensus TODO} \label{sec:raft}
...
\begin{table}[ht]
    ...
    \begin{tabular}{|>{\raggedright\arraybackslash}p{4cm}|c|c|c|c|c|}
        ...
        \hline
        Safety & \cmark & \xmark & \text{Deadlock} & \xmark & \text{Deadlock} \\
        \hline
